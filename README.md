...
ยังไม่มีรูปให้ลอง — ในแพ็กเกจไม่มีรูปใบหน้าติดมาเลย ทั้งโดยตั้งใจและโดยจำเป็น
ใช้รูปของคุณเอง หรือดึงชุดภาพสาธารณสมบัติมาลอง

```bash
python fetch-reference-faces.py --out tests/faces-public
```

ได้ภาพบุคคลสาธารณะจาก Wikimedia Commons ราวสิบรูป คนละ 2–3 รูป
พร้อมบันทึกไลเซนส์ไว้ใน `CREDITS.md` ข้างๆ

**ชุดนี้ใช้ยืนยันว่าท่อทำงานเท่านั้น ไม่ใช่ข้อมูลปรับเทียบ** เป็นภาพถ่ายสตูดิโอและ
ภาพข่าว ส่วนคนที่จะใช้งานจริงนั่งอยู่หน้าเว็บแคมในแสงห้องของตัวเอง
threshold ที่วัดจากชุดนี้จะวัดจากการกระจายตัวที่ผิด — และแย่กว่าการใช้ค่าตั้งต้น
เพราะมันดูเหมือนผ่านการตรวจสอบมาแล้ว

จากนั้นลองลงทะเบียนแล้วเทียบ

```bash
curl -s localhost:9000/embed -H "X-Face-Key: $FACE_API_KEY" -F image=@tests/faces-public/harris/1.jpg
```

เอาค่า `embedding` ที่ได้ไปใส่ใน `/verify` เทียบกับรูปที่สองของคนเดียวกัน
ควรได้ `decision: pass` และเทียบกับคนอื่นควรได้คะแนนต่ำลงชัดเจน

> ที่วัดได้จากชุดนี้: คนเดียวกัน 0.74–0.79 คนละคน 0.11–0.31
> สังเกตว่า 0.31 **ตกในช่วง `review` ไม่ใช่ `fail`** ทั้งที่เป็นคนละคน —
> นี่คือเหตุผลที่ต้องปรับเทียบก่อนใช้จริง ดูหัวข้อ "การปรับ threshold"
...
## โมดูลฝั่งเบราว์เซอร์

`browser/` มี JavaScript ที่ทำงานคู่กับบริการนี้ — ตรวจจับเม้าหยุดนิ่ง
การออกจากหน้าต่าง การสลับแท็บ การไม่พบคนหน้ากล้อง และการยืนยันตัวตนเป็นระยะ
แล้วหยุดสื่อ แสดงข้อความ และส่งบันทึกไปที่ระบบของคุณ

ไม่พึ่งไลบรารีใด ไม่ต้อง build ดู [browser/README.md](browser/README.md)

**โมดูลเหล่านี้ไม่เรียกบริการนี้โดยตรง** และไม่ควรเรียก — เบราว์เซอร์ยิงไป
ที่ backend ของคุณ ซึ่งเป็นที่เดียวที่ถือ `FACE_API_KEY` และเป็นที่ที่บันทึก
ผลการตัดสิน บันทึกที่เบราว์เซอร์เขียนเองคือบันทึกที่ผู้ถูกเฝ้าดูเป็นคนเขียน
...
browser/             โมดูล JavaScript ฝั่งเบราว์เซอร์ (ดู browser/README.md)
```
